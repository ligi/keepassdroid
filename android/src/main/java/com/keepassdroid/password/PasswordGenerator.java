...
public class PasswordGenerator {
    ...
    public String generatePassword(int length,
                                   ...
                                   boolean space,
                                   boolean specials,
                                   boolean brackets) throws IllegalArgumentException {
        if (length <= 0) throw new IllegalArgumentException(cxt.getString(R.string.error_wrong_length));
    ...
    public String getCharacterSet(boolean upperCase,
                                  boolean lowerCase,
                                  boolean digits,
                                  boolean minus,
                                  boolean underline,
        ...
        if (digits) charSet.append(digitChars);

        if (minus) charSet.append(minusChars);

        if (underline) charSet.append(underlineChars);

        if (space) charSet.append(spaceChars);

        if (specials) charSet.append(specialChars);

        if (brackets) charSet.append(bracketChars);
